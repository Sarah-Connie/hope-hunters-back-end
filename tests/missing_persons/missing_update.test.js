...
describe('missing/update allows a police user, admin user or the document owner to change a document in the missingpersons collection', () => {
	test("Route returns the updated document. Route also allows a police user to modify the document", async () => {
		...
		const updatedResponse = await request(app)
                                .put(`/missing/update/` + newResponse.body._id)
                                .send({
                                    areaSuspectedToBe: "Maitland",
	                                distinctiveFeatures: "tattoo on right forearm"
								})
        ...
        expect(updatedResponse.body.areaSuspectedToBe).toBe('Maitland');
        expect(updatedResponse.body.distinctiveFeatures).toBe('tattoo on right forearm');
	});
