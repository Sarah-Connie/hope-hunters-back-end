...
const policeUsersConfirmation = async (request, response) => {
    ...
    let addJWT = await PoliceUser.findOneAndUpdate({email: request.params.email}, {jwt: jwtoken}, {new: true})
                    .catch(error => { 
                        return response.status(404).json({error:'Unable to find user document.'});
                    }) 
